 ...
 */
public class PolymorphicDispatcherTreeSet<RT> {
	...
	public PolymorphicDispatcherTreeSet(final String methodName, final List<? extends Object> targets) {
		this(methodName, 1, 1, targets);
	}

	public PolymorphicDispatcherTreeSet(final String methodName, final int minParams, final int maxParams, final List<? extends Object> targets) {
	...
	public PolymorphicDispatcherTreeSet(final List<? extends Object> targets, Predicate<Method> methodFilter) {
		this(targets, methodFilter, new DefaultErrorHandler<RT>());
	}

	public PolymorphicDispatcherTreeSet(final List<? extends Object> targets, Predicate<Method> methodFilter, ErrorHandler<RT> handler) {
	...
	private Collection<MethodDesc> getDeclaredMethodsOrderedBySpecificParameterType() {
		SortedSet<MethodDesc> cachedDescriptors = new TreeSet<MethodDesc>(new Comparator<MethodDesc>() {
			...
			public int compare(MethodDesc o1, MethodDesc o2) {
				int compare = PolymorphicDispatcherTreeSet.this.compare(o1, o2);
				if (compare != 0) {
					return compare;
				}
